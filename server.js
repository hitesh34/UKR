...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'hiteshmansinghani@gmail.com',
    pass: 'vett dvqz takb wwkg',
  },
});
...
async function sendEmailWithPDF(pdfBuffer, subject, email) {
  try {
    const info = await transporter.sendMail({
      from: 'hiteshmansinghani@gmail.com',
      to: email,
      subject: subject,
...
async function generatePDF(htmlContent) {
  const browser = await playwright.chromium.launch();  const page = await browser.newPage();
